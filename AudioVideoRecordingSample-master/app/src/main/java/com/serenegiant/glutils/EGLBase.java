...
public class EGLBase {	// API >= 17
	...
	public MyEglSurface createFromSurface(final Object surface) {
		if (DEBUG) Log.v(TAG, "createFromSurface:");
		final MyEglSurface eglSurface = new MyEglSurface(this, surface);
		eglSurface.makeCurrent();
		return eglSurface;
	}

	public MyEglSurface createOffscreen(final int width, final int height) {
		if (DEBUG) Log.v(TAG, "createOffscreen:");
		final MyEglSurface eglSurface = new MyEglSurface(this, width, height);
		eglSurface.makeCurrent();
		return eglSurface;
	}
	...
	private void init(EGLContext shared_context, final boolean with_depth_buffer, final boolean isRecordable) {
        ...
        eglWrapper.checkAlreadySettedUp();
	 ...
	 * @return
	 */
	public boolean makeCurrent(final EGLSurface surface) {
//		if (DEBUG) Log.v(TAG, "makeCurrent:");
	...
	public int swap(final EGLSurface surface) {
//		if (DEBUG) Log.v(TAG, "swap:");
    ...
    public EGLContext createContext(final EGLContext shared_context) {
//		if (DEBUG) Log.v(TAG, "createContext:");
    ...
    public EGLSurface createWindowSurface(final Object nativeWindow) {
    ...
    public void checkEglError(final String msg) {
        int error;
